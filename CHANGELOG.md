...
**A published book is 263 KB smaller, a story grid retargets its cells several times faster, and four plugin API members are now deprecated with replacements.**

Most of this release is invisible in the good way: books start faster, build lighter, and the dev server stops leaving watchers and Nuxt instances behind. The parts that ask something of you are two breaking changes and one deprecation, all in the plugin and setup surfaces, and all listed with an Upgrading line below.

It is a `minor` because two commits in the range are a `feat`: `api.watch` ([#862](https://github.com/poveste-dev/poveste/pull/862)) and `setupSvelte` ([#798](https://github.com/poveste-dev/poveste/pull/798)). The version comes from the commits, not from the milestone.
...
- **`api.watch`, a file watcher Poveste closes for you** ([#356](https://github.com/poveste-dev/poveste/issues/356)). `api.watch(paths, callback)` reports `add`, `change` and `unlink`, returns a function that stops it early, and is closed when the dev server closes — including when the server fails to start, which is where a plugin's own watcher used to leak. A watch requested after close is a no-op, a build gets none, and a callback that throws is logged with your plugin's name rather than becoming an unhandled rejection.
- **Four plugin API members are deprecated** ([#356](https://github.com/poveste-dev/poveste/issues/356)). `api.colors`, `api.path`, `api.fs` and `api.watcher` keep working for the rest of 0.x and become removable at 1.0. Import `picocolors`, `node:path` (or `pathe`) and `node:fs` directly, and watch files with the new `api.watch`, which Poveste closes when the dev server closes, so a plugin has nothing to release.
- **`setupSvelte` is accepted as a setup hook name** ([#157](https://github.com/poveste-dev/poveste/issues/157)). The unnumbered name brings Svelte into line with Vue's `setupVue3`/`setupVue` pair. The numbered names keep working and keep precedence, so nothing has to be renamed.
- **A published book no longer carries Vue Devtools** ([#791](https://github.com/poveste-dev/poveste/issues/791)). Pinia imports the devtools API at the top level, and a built book kept it because `__VUE_PROD_DEVTOOLS__` was on, so every reader downloaded integration code a published book cannot use. A book built here went from 5,335,346 to 5,071,944 bytes — **263,402 bytes smaller** — and the bundle-size ceilings were lowered to fail if it comes back. The dev server keeps Devtools.
- **Syntax grammars load when a fence names one** ([#825](https://github.com/poveste-dev/poveste/issues/825)). `poveste dev` and `poveste build` loaded all 346 of shiki's bundled grammars before starting, whether or not the book had any markdown. The highlighter starts empty now and loads a fence's grammar, and whatever that grammar embeds, on first use.
- **A grid retargets its cells with work proportional to the cell, not the story** ([#873](https://github.com/poveste-dev/poveste/issues/873), [#888](https://github.com/poveste-dev/poveste/issues/888)). Showing one variant in a grid cell rendered every variant the story has. On a 1000-variant story a retarget went from 19.5 ms to 1.3 ms of Vue runtime, and from 7.7 ms to 2.2 ms of main-thread time under Svelte, with the worst frame of a fast fling roughly halved in each. Nothing to do: rebuild your book and it is faster.
- **Story collection runs on Vite's module runner** ([#809](https://github.com/poveste-dev/poveste/pull/809)), and the config is loaded by Vite's own loader rather than jiti ([#870](https://github.com/poveste-dev/poveste/issues/870)). Two fewer dependencies in the graph that reads your stories, and one loader behind both your `vite.config` and your `poveste.config`.

### 🩹 Fixes

- **`autoPropsDisabled` works where it was documented and did nothing** ([#890](https://github.com/poveste-dev/poveste/issues/890)). Set on a Vue *story*, it never reached the realm that renders a variant, so the controls panel still offered prop controls for every variant. Set on a Svelte *variant*, it was recorded nowhere at collection and applied only if one mount happened to finish before another, so it worked about half the time. Both now do what the reference pages have always said.
- **A config using top-level await says what to do about it** ([#870](https://github.com/poveste-dev/poveste/issues/870)). A `poveste.config.ts` in a package that is not `"type": "module"` is bundled to CommonJS, which has no top-level await. It used to fail with a bundler error naming neither the cause nor the fix; it now says the config uses top-level await and that setting `"type": "module"` in that package's `package.json` is what supports it.
- **`__dirname` in a config resolves symlinks** ([#870](https://github.com/poveste-dev/poveste/issues/870)). A config reached through a symlinked path now sees the real path rather than the link, which is what Node's own loader reports. A config building paths from `__dirname` inside a symlinked checkout gets the resolved location.
- **A Nuxt story gets the real `useNuxtApp`** ([#439](https://github.com/poveste-dev/poveste/issues/439)). The plugin shipped a stub answering `runWithContext` and nothing else, and returning an object — so a story could call the composable, read `undefined` off it and never crash. Values a Nuxt plugin provided, and the runtime config, were both missing. The stub is gone, and an example plugin plus a spec now assert both halves. Inherited from histoire ([histoire#666](https://github.com/histoire-dev/histoire/issues/666), [histoire#710](https://github.com/histoire-dev/histoire/issues/710)).
- **`poveste dev` closes what it opened** ([#866](https://github.com/poveste-dev/poveste/issues/866), [#867](https://github.com/poveste-dev/poveste/issues/867), [#877](https://github.com/poveste-dev/poveste/issues/877), [#878](https://github.com/poveste-dev/poveste/issues/878)). A failed start left both Vite servers and their watchers running; every config restart leaked the story watcher and piled up change handlers bound to servers that had closed; a restart during collection printed errors from the server it had just closed; and the teardown a plugin registers while the config resolves ran as soon as the server started, which closed `@poveste/plugin-nuxt`'s Nuxt instance at the beginning of every session. Ctrl-C now runs those teardowns and exits deliberately.
- **A failed build stops leaking a file watcher** ([#849](https://github.com/poveste-dev/poveste/issues/849)). `poveste build` started a story watcher it never used, and on macOS a failed build left its fsevents stream open, so the process stayed alive with nothing to do.
- **A sandbox takes messages only from the book's own origin** ([#379](https://github.com/poveste-dev/poveste/issues/379)). `__sandbox.html` is a public URL in every published book, and any page embedding it was its `parent`. It now ignores messages from any other origin, so an embedding page cannot write story state, retarget a realm or restyle it.
- **Opening a story nobody has opened shows its first variant** ([#728](https://github.com/poveste-dev/poveste/pull/728)), and a story's error clears when the realm that reported it says so ([#746](https://github.com/poveste-dev/poveste/pull/746)).
- **The JSON control survives a shared reference** ([#801](https://github.com/poveste-dev/poveste/pull/801)). A value whose graph reaches the same object twice sent the control walking it without end.
- **Controls, events and wrapped components render as written** ([#838](https://github.com/poveste-dev/poveste/pull/838), [#747](https://github.com/poveste-dev/poveste/pull/747), [#799](https://github.com/poveste-dev/poveste/pull/799)). A control wrapped by a plugin renders through Vue's dynamic `Component` type; the source pane shows an event handler the way its author wrote it; and a Svelte story's `setupApp` and `responsiveDisabled` reach its explicit variants, not only an implicit one.
- **The panel is usable from a keyboard and a screen reader** ([#771](https://github.com/poveste-dev/poveste/pull/771), [#829](https://github.com/poveste-dev/poveste/pull/829), [#848](https://github.com/poveste-dev/poveste/pull/848)). Five icon-only actions are buttons that announce a save and a copy, the preset picker is a select-only combobox with rename and delete beside it, and the source panel's loading and failure states are announced rather than silent.
- **Nuxt collection answers its own fetches** ([#778](https://github.com/poveste-dev/poveste/pull/778)) and starts no file watchers in the instance a build reads its config from ([#802](https://github.com/poveste-dev/poveste/pull/802)). Percy waits for the story before serialising a sandbox ([#753](https://github.com/poveste-dev/poveste/pull/753)).
- **`poveste build` says where it looked when it finds no stories** ([#743](https://github.com/poveste-dev/poveste/pull/743)), and two inherited debug lines no longer print on every run ([#714](https://github.com/poveste-dev/poveste/pull/714)).

### 📖 Documentation

- **Every page's `.html` twin redirects to its clean URL** ([#840](https://github.com/poveste-dev/poveste/pull/840)), checked against the live site rather than the build output.
- **The pages a reader arrives on were written for the arrival**: what the fork adds, for a histoire reader ([#776](https://github.com/poveste-dev/poveste/pull/776)); what Poveste does not do, for a Storybook user ([#792](https://github.com/poveste-dev/poveste/pull/792)); and the home page's Nuxt and SvelteKit tiles now go to those frameworks' own guides ([#807](https://github.com/poveste-dev/poveste/pull/807)).
- **What a deprecation promises is written down** ([#789](https://github.com/poveste-dev/poveste/pull/789)), including that an untagged one is not one. This release's deprecations follow it.
- **Story isolation explains itself** ([#784](https://github.com/poveste-dev/poveste/pull/784)): why `@scope` rather than a shadow root.
...
**If you maintain a plugin that uses `api.colors`, `api.path`, `api.fs` or `api.watcher`**, they still work and will through 0.x. Move to `picocolors`, `node:path`, `node:fs` and `api.watch` when it suits you; only `api.watch` changes what you have to clean up, because Poveste closes it with the dev server.

**If your `poveste.config.ts` uses top-level await**, set `"type": "module"` in the `package.json` nearest it. That config previously loaded through jiti, which allowed it; the error now names the fix.
...
- **`sendEvent` returns a promise in a built book** ([#688](https://github.com/poveste-dev/poveste/pull/688)). `sendEvent` from `poveste/plugin` is typed to return a `Promise`. A built book has no plugin host, and in that case the function returned `undefined`, so `sendEvent(...).then(...)` threw in a built book while `await` happened to work. It now resolves to `undefined`, as its signature promised.
...
* Failed to load /virtual:$poveste-generated-global-setup-resolved ([e147acd](https://github.com/Akryum/histoire/commit/e147acd55555ecf230785fa7d1ed46a36b4fb766))
