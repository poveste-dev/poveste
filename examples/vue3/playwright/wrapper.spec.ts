...
test.describe('wrapper', () => {
  test('applies the global wrapper to a basic story', async ({ page }) => {
    ...
    await expect(iframe.locator('.poveste-generic-render-story .global-wrapper')).toHaveCSS('padding', '4px')
  })
  ...
  test('respects story-level wrapper meta', async ({ page }) => {
    ...
    await expect(iframe.locator('.poveste-generic-render-story .global-wrapper')).not.toHaveCSS('padding', '4px')
