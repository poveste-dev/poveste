...
describe('listComponentFiles', () => {
  it('leaves out components git ignores, from the root .gitignore and a nested one', async () => {
    const root = tree({
      ...
      'app/src/Button.vue': '',
      'app/src/generated/Output.vue': '',
      'app/scratch/Draft.vue': '',
      'app/scratch/Kept.vue': '',
    })

    const files = await listComponentFiles('', [], 10, join(root, 'app'))

    expect(files.sort()).toEqual(['scratch/Kept.vue', 'src/Button.vue'])
  })
  ...
  it('still skips node_modules and the patterns it is given', async () => {
    const root = tree({
      '.git/HEAD': 'ref: refs/heads/main\n',
      'src/Button.vue': '',
      'src/Button.story.vue': '',
      'node_modules/pkg/Dep.vue': '',
    })

    expect(await listComponentFiles('', ['**/*.story.vue'], 10, root)).toEqual(['src/Button.vue'])
  })
})
