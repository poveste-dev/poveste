...
const trackedWindows = shallowRef<Window[]>([])
...
/**
 * Opt an additional window (typically a sandbox iframe `contentWindow`) into
 * shortcut tracking, so keys pressed while it has focus still reach the app.
 */
export function trackWindow(target: Window): () => void {
  if (trackedWindows.value.includes(target)) {
    return () => {}
  }
