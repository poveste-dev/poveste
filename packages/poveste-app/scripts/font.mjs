...
if (process.argv[1]?.endsWith('font.mjs')) {
  ...
  // Removed as well as written. A bump that folds or renames a subset would
  // otherwise leave the old file behind, and `files: ['dist', 'src']` publishes
  // it twice — bytes nobody asked for, in the change that exists to stop that.
  for (const existing of readdirSync(fonts, { withFileTypes: true })) {
    // Files only: `rmSync` without `recursive` throws on a directory, and it
    // would do so after the sheet is already rewritten.
    if (existing.isFile() && existing.name !== 'LICENSE' && !files.includes(existing.name)) {
      rmSync(join(fonts, existing.name))
    }
  }
