...
It passes Quasar's plugins through whole — unusually, since the Nuxt plugin drops six, and removing Quasar's Vue plugin in particular fails config resolution because `@quasar/vite-plugin` asserts one precedes it. It also sets `ssr.noExternal: [/quasar/]` for story collection, so that Quasar's source is transformed rather than externalised and the `__QUASAR_VERSION__` marker it writes actually lands.

Neither is something to copy into your own config. The setup, the boot-file requirement that costs a day when it is missed, and what is checked against what are on the framework's own page: **[Getting started with Quasar](./quasar/getting-started.md)**.
