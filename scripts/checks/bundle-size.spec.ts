...
describe('measurements', () => {
  ...
  it('reports every ceiling', () => {
    expect(measurements(chunks, LIMITS)).toEqual(expect.arrayContaining([
      ...
      'vendor-def.js 1413 KB / 1650 KB',
      'whole book 2957 KB / 5250 KB',
    ]))
  })

  it('totals the whole book for the ceiling that has no prefix', () => {
    expect(measurements(chunks, LIMITS)).toContain('whole book 2957 KB / 5250 KB')
  })
  ...
  it('gives each matched chunk its own line', () => {
    ...
    expect(measurements(split, LIMITS.slice(1, 2))).toEqual([
      'vendor-a.js 1400 KB / 1650 KB',
      'vendor-b.js 1300 KB / 1650 KB',
    ])
  })
