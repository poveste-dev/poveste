...
export const WITHOUT_FLOOR: Record<string, string> = {
  ...
  'check-recipes.ts': 'guarded per recipe: a missing section and a block count that does not match the files are both reported by name, so a parse that yields nothing fails once per recipe rather than passing. `section` returning nothing and the count from `tsBlocks` are asserted directly, and `check-recipes.spec.ts` runs the check as a process over a page that has lost its section, asserting the exit (#719)',
  ...
  'check-example-wiring.ts': 'three guards, all three in `main()` because each is the reaction to a read it just did: no `example:` matrix, the guide missing, no example table in it. `check-example-wiring.spec.ts` runs the check as a process over a workflow with no matrix and asserts the exit, and `conformanceBooks` is asserted directly (#719)',
  'check-docs-site.ts': 'separates a missing build from a built one by name, and asserts specific paths are present, so an empty build fails before anything counts. That is in `checkBuild`, and `check-docs-site.spec.ts` runs the check as a process over a root with no build, asserting the exit (#719)',
  'check-bundle-size.ts': 'separates "could not read the directory" from "no book in it" deliberately, and exits 1 for both. The half that finds the book is `findBook`, asserted directly, and `check-bundle-size.spec.ts` runs the check as a process over an example with no built book, asserting the exit (#719)',
  'check-conformance-config.ts': 'guards an empty spec list in `specProblems` and an empty book list through `conformanceBooks`, both asserted directly. The empty defaults list is guarded in `main()`, and `check-conformance-config.spec.ts` runs the check as a process over a defaults file with no presets in it, asserting the exit (#719)',
  'check-doc-coverage.ts': 'reports an unbuilt tree by name, because a partial run over resolving shims is the failure it exists to catch. The guard is in `main()`, over the two lists the run produced, and `check-doc-coverage.spec.ts` runs the check as a process over a tree where nothing can be measured, asserting the exit (#719)',
  ...
  'check-preview-position.ts': 'guards an empty group list in `problemsIn`, asserted directly. The empty file list is guarded in `main()`, as the reaction to a glob over the app source matching nothing, and `check-preview-position.spec.ts` runs the check as a process over an app source with no component, asserting the exit (#719)',
